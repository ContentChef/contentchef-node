 ...
 * @returns
 */
export function configurePreviewMethods(
  config: ISDKConfiguration,
  targetDateResolver: ITargetDateResolver,
): interfaces.GetPreviewChannelMethods {
  return (
    apiKey: string,
    channel: string,
    state: PublishingStatus = PublishingStatus.Live,
    locale?: string,
  ) => {
    return getPreviewChannelMethods(
      config.spaceId,
      channel,
      state,
      { ...defaultConfig, ...config, apiKey, locale },
      targetDateResolver,
    );
  };
}
 ...
 * @returns
 */
export function configureOnlineMethods(
  config: ISDKConfiguration,
): interfaces.GetOnlineChannelMethods {
  return (apiKey: string, channel: string, locale?: string) => {
    return getOnlineChannelMethods(config.spaceId, channel, {
      ...defaultConfig,
      ...config,
      apiKey,
      locale,
 ...
 * @param {ISDKConfiguration} config
 */
export function createOnlineContentRequest(
  spaceId: string,
  channel: string,
 ...
 * @param {ISDKConfiguration} config
 */
export function createPreviewContentRequest(
  spaceId: string,
  channel: string,
  ...
  return async <T extends object>(
    ...
    const searchParams: URLSearchParams =
      createGetContentRequestURLSearchParams(params, targetDate);
    return executeFetchRequest(config, url, searchParams);
  };
}
 ...
 * @param {ISDKConfiguration} config
 */
export function createOnlineSearchRequest(
  spaceId: string,
  channel: string,
  config: IChannelConfiguration,
  locale?: string,
...
export function createPreviewSearchRequest(
  spaceId: string,
  channel: string,
  state: PublishingStatus,
  config: IChannelConfiguration,
...
function maybeAddToURLSearchParams(
...
function createGetContentRequestURLSearchParams(
  params:
    | interfaces.GetContentPreviewConfig
...
export async function executeFetchRequest<T>(
  config: IChannelConfiguration,
  url: string,
  params: URLSearchParams,
...
export function getPreviewChannelMethods(
  ...
  const search = createPreviewSearchRequest(
    spaceId,
    channel,
    state,
    config,
  ...
  const localizedContent = createPreviewContentRequest(
    spaceId,
    channel,
    state,
    config,
  ...
  const localizedSearch = createPreviewSearchRequest(
    spaceId,
    channel,
    state,
    config,
 ...
 * @returns
 */
export function getOnlineChannelMethods(
  spaceId: string,
  channel: string,
  config: IChannelConfiguration,
): interfaces.IChannelMethods {
  ...
  const localizedContent = createOnlineContentRequest(
    spaceId,
    channel,
    config,
    config.locale,
 ...
 * @returns
 */
export function getOnlineEndpoint(
  spaceId: string,
  method: interfaces.ContentRequestMethod,
  channel: string,
  locale?: string,
) {
