...
## [Unreleased]
### Added
- `dimensions` param on `content()` and `search()` (online and preview), taking the visitor's
  assignment as `{ [dimensionMnemonicId]: key }` and sending one `dimensions=<dimension>:<key>`
  query param per entry
